...
func (dir *Directory) WithSymlink(ctx context.Context, srv *dagql.Server, target, linkName string) (*Directory, error) {
	...
	err = op.Mount(ctx, newRef, func(root string) error {
		return os.Symlink(target, path.Join(root, linkName)) // TODO (dont merge as-is), test for cases where the target is absolute, e.g. user does somedir | with-symlink /existing-file /new-symlink
	})
	if err != nil {
		return nil, err
	}
